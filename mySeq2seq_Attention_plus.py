...
class MyModel(nn.Module):
    def __init__(self,eng_to_index, index_to_eng, ch_to_index, index_to_ch,embedding_num,hidden_num):
        ...
        self.decoder = nn.LSTM(input_size=embedding_num+1,hidden_size=hidden_num,num_layers=1,bidirectional=False,batch_first=True)
        self.W_k = nn.Linear(hidden_num,1, bias=False)
        self.W_q = nn.Linear(hidden_num,1, bias=False)
        self.W_v = nn.Linear(hidden_num,1, bias=False)
    ...
    def forward(self,encoder_input,decoder_input,label,decoder_hidden=None):
        ...
        query = self.W_q(decoder_hidden.permute(1,0,2))
        key = self.W_k(encoder_output)
        attscore = query.unsqueeze(2) + key.unsqueeze(1)
        attweight = torch.tanh(attscore).squeeze(-1)
        value = self.W_v(torch.bmm(torch.softmax(attweight,dim=-1), encoder_output))
...
def translate(mydataset,ch_to_index,index_to_ch):
    ...
    for eng_embedding,_,_ in test_dataloader:
        ...
        while True:
            ...
            query = model.W_q(decoder_hid[0].permute(1, 0, 2))
            key = model.W_k(encoder_output)
            attscore = query.unsqueeze(2) + key.unsqueeze(1)
            attweight = torch.tanh(attscore).squeeze(-1)
            value = model.W_v(torch.bmm(torch.softmax(attweight,dim=-1), encoder_output))
            context = torch.repeat_interleave(value, decoder_input.shape[1], dim=1)
            feature = torch.cat((decoder_input, context), dim=2)
...
if __name__ == '__main__':
    ...
    for e in range(epoch):
        ...
        for eng_embedding,x_embedding,y_embedding in dataloader:
            loss,attweight,decode_hidden = model(eng_embedding,x_embedding,y_embedding,decode_hidden)
            decode_hidden = decode_hidden.detach()
            loss.backward()
            opt.step()
            opt.zero_grad()
